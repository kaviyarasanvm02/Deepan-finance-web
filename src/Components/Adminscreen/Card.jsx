...
export default function Cardss() {
   ...
   const [successOpen, setSuccessOpen] = useState(false);
    ...
    const [formData, setFormData] = React.useState({
      ...
      console.log(cardDetailsData,"cardDetailsDatasdjkjhgfdsdfghjhgfd");
  ...
  return (
   <AdminContentPart>
    <Box sx={{ display:"flex", flexDirection:"row", justifyContent:"flex-end"}}>
       <Button variant="outlined" startIcon={<ControlPointIcon />} onClick={handleOpen} disabled={cardOpen}>
        Add Cards
      </Button>
      </Box>
      <Dialog
      ...
      >
        <Box sx={{padding:"20px"}}>
         <Grid container spacing={3}>
           <Grid item xs={12}>
           <form onSubmit={editingCardId ? (e) => { e.preventDefault(); UpdateHeader(editingCardId) } : createHeader}>
         ...
         <Grid container spacing={4} sx={{ display: "flex", flexDirection: "row", marginTop:"10px" }}>
    ...
    <Grid item xs={12} sm={6} md={4} lg={3} key={e.id}>
      <Card>
        ...
        <Card.Body className='cardbody'>
          ...
          <Editbtn onClick={() => handleOpenEditModal(e.id)}><MdEdit /></Editbtn>
        </Card.Body>
      </Card>
    </Grid>
  ))}
